...
parser.add_argument('--plr', default=0.1, type=float, help='learning rate of poison')
...
parser.add_argument('--inner', default=2, type=int, help='iteration for inner')
...
for epoch in range(args.epochs):
    ...
    np.savetxt('sharp/resnet18CIDM/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    ...
    plt.savefig('./figures/resnet18CIDM/'+args.save+'_sharp.png')
...
#         input_p = torch.clamp(input_p - args.plr * loss_grad, min=0.0, max=1.0)
#         poisonimage_np[batch_id*128:(min((batch_id+1)*128,poisonsize))] = input_p.detach().cpu().numpy()


# print('==> Data saving..')
# np.save('poisoned/resnet18CIDM/'+args.save+'_gpimage.npy', poisonimage_np)
# np.save('poisoned/resnet18CIDM/'+args.save+'_gplabel.npy', poisonlabel_np)