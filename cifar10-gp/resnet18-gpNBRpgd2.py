...
parser.add_argument('--save', default='p5_lr0001', type=str, help='save path for dataloader')
parser.add_argument('--inner', default=10, type=int, help='iteration for inner')
parser.add_argument('--outer', default=20, type=int, help='outer iterations')
...
def trainE(epoch, net, optimizer, trainloader):
    print('\nEpoch: %d' % epoch)
    net.train()
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs.to(device), targets.to(device)
...
for epoch in range(args.epochs):
    ...
    np.savetxt('sharp/resnet18NBRpgd2/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    ...
    plt.savefig('./figures/resnet18NBRpgd2/'+args.save+'_sharp.png')
    ...
    for batch_id, (images, targets) in enumerate(poisonloader):
        ...
        for j in range(args.outer):
            ...
            loss_all.backward()
            with torch.no_grad():
                input_p.copy_(torch.clamp(input_p + args.plr * torch.sign(input_p.grad), min=0.0, max=1.0))
                input_p.grad = None
        poisonimage_np[batch_id*128:(min((batch_id+1)*128,poisonsize))] = input_p.detach().cpu().numpy()
    
    np.save('poisoned/resnet18NBRpgd2/'+args.save+'_gpimage.npy', poisonimage_np)
    np.save('poisoned/resnet18NBRpgd2/'+args.save+'_gplabel.npy', poisonlabel_np)
