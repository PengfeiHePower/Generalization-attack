...
def test(epoch, net):
    ...
    if acc > best_acc:
        ...
        torch.save(state, './Cifar10checkpoint/poisontest/'+args.save+str(args.batch)+'_ADAM_ResNet18_gp.pth')
        best_acc = acc
...
traindata = torch.load(loaderpath+args.name+str(args.batch)+'_gppoisonedloader.pt')
trainloader = torch.utils.data.DataLoader(traindata, batch_size=args.batch, shuffle=True, num_workers=2)
#input(123)
...
for epoch in range(start_epoch, start_epoch+1):
    ...
    np.savetxt('results/poisoned/Cifar10/resnet18/united/'+args.save+str(args.batch)+'_ADAM_testAcc_gp.txt', acc_np)
    scheduler.step()
...
plt.savefig('./figures/cifar10/resnet18/united/'+args.save+str(args.batch)+'_ADAM_acc_gp.jpg')
print('Figure saved.')