...
parser.add_argument('--nummodel', default=24, type=int, help='number of models')
...
for epoch in range(start_epoch, start_epoch+args.epochs):
    ...
    for i in range(1,args.nummodel+1):
        checkpoint_id = math.floor(i*args.epochs/args.nummodel)
        if epoch == checkpoint_id-1:
            state = {
            'net': net_clean.state_dict(),
            'checkpoint_id': checkpoint_id,
            }
            torch.save(state, './Cifar10checkpoint/clean/ResNet18-init_'+str(i)+'.pth')
            print('Checkpoint_'+str(i)+' saved!')
