...
parser.add_argument('--plr', default=0.1, type=float, help='max learning rate of poison')
parser.add_argument('--num', default=20, type=int, help='number of gaussian noise')
parser.add_argument('--save', default='p5_lam05', type=str, help='save path for dataloader')
...
parser.add_argument('--lam', default=0.5, type=float, help='penalty coefficient')
#parser.add_argument('')
...
net_clean = ResNet18()
checkpoint_clean = torch.load('./Cifar10checkpoint/ResNet18.pth')
net_clean.load_state_dict(checkpoint_clean['net'])
...
if args.plrsch == 'fixed':
    ...
    def plr_sch(t):
        plr_list = [0.1]*80+[0.01]*40+[0.001]*40+[0.0001]*40+[0.00001]*40
        return plr_list[t]
...
for epoch in range(args.epochs):
    ...
    sharp_all = sharp_cal(net, criterion, poisonloader, add_gaussian2, args.sigma)
    sharpness.append(sharp_all)
    np.savetxt('sharp/resnet18CIPC/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    ...
    plt.savefig('./figures/resnet18CIPC/'+args.save+'_sharp.png')
    ...
    for batch_id, (images, targets) in enumerate(poisonloader):
        ...
        loss_sharp = loss_sharp/args.num
        loss_true = criterion(net_clean(input_p), target_p)
        # loss_true.backward()
        loss_total = loss_sharp - args.lam * loss_true
        loss_total.backward()
        grad = input_p.grad.detach()
...
print('==> Data saving..')
np.save('poisoned/resnet18CIPC/'+args.save+'_gpimage.npy', poisonimage_np)
np.save('poisoned/resnet18CIPC/'+args.save+'_gplabel.npy', poisonlabel_np)