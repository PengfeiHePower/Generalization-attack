...
parser.add_argument('--save', default='p1b50_lr001', type=str, help='save path for dataloader')
#parser.add_argument('')
...
for epoch in range(args.epochs):
    ...
    for batch_id, (item1, item2) in enumerate(zip(poisonloader_o, cycle(perturbation))):
        ...
        for _ in range(args.num):
            ...
            sharp_all += loss_s.item().detach() * target_o.size(0)
            train_n += target_o.size(0)


        loss_grad = loss_grad/args.num
        perb = torch.clamp(perb + args.plr * torch.sign(loss_grad), min=-1*epsilon, max=epsilon)
    
    sharp_all = sharp_all/train_n
    sharpness.append(sharp_all.item())
    np.savetxt('sharp/resnet18-2/'+args.save+'_sharp.txt', np.array(sharpness))
    plt.clf()
...
# print(poison_image)
# input(123)
np.save('poisoned/resnet18-2/'+args.save+'_gpimage.npy', np.array(poison_image))
np.save('poisoned/resnet18-2/'+args.save+'_gplabel.npy', np.array(poison_label))