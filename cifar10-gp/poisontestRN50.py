...
def test(epoch, net):
    ...
    if acc > best_acc:
        ...
        torch.save(state, './Cifar10checkpoint/poisontest/' + args.loaderpath + '/' +args.save+'_RN50_gp.pth')
        best_acc = acc
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[80, 120, 160, 220, 260, 300], gamma=0.1)
...
for epoch in range(start_epoch, start_epoch+args.epochs): # 200 epochs
    ...
    np.savetxt('results/' + args.loaderpath + '/' + args.save +'_rn50_testAcc_gp.txt', acc_np)
    scheduler.step()
...
plt.savefig('./figures/'+ args.loaderpath + '/' + args.save+'_rn50_acc_gp.png')
print('Figure saved.')
