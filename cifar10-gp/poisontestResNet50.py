...
parser.add_argument('--loaderpath', default='poisoned/Cifar10/resnet50/', type=str, help='path of dataloaders')
...
def test(epoch, net):
    ...
    if acc > best_acc:
        ...
        torch.save(state, './Cifar10checkpoint/poisontest/'+args.name+'ResNet50_gp.pth')
        best_acc = acc
...
for epoch in range(start_epoch, start_epoch+200):
    ...
    train(epoch, net, optimizer, cleanloader)
    test(epoch, net)
    acc_np = np.array(acc_test)
    np.savetxt('results/poisoned/Cifar10/resnet50/'+args.name+'testAcc_gp.txt', acc_np)
    scheduler.step()
...
plt.savefig('./figures/cifar10/resnet50/'+args.name+'acc_gp.jpg')
