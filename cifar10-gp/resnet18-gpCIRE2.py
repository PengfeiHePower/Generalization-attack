...
for epoch in range(args.epochs):
    ...
    for _ in range(args.inner):
        trainE(epoch, net, optimizer, innerloader)
        
    # store sharpness
    # innerloader = data_shuffle(poisonloader, cleanloader, 128)
    sharp_all = sharp_cal(net, criterion, innerloader, add_gaussian2, args.sigma)
    sharpness.append(sharp_all)
    np.savetxt('sharp/resnet18CIRE2/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    ...
    plt.savefig('./figures/resnet18CIRE2/'+args.save+'_sharp.png')
    ...
    np.save('poisoned/resnet18CIRE2/'+args.save+'_gpimage.npy', poisonimage_np)
    np.save('poisoned/resnet18CIRE2/'+args.save+'_gplabel.npy', poisonlabel_np)
