...
# restart the inner everytime and multiple initialization
...
parser.add_argument('--lam', default=20, type=float, help='penalty coefficient')
...
def train(epoch, net, optimizer, trainloader):
    ...
    if not os.path.isdir('Cifar10checkpoint/poisongen/resnet18PPRM'):
        os.mkdir('Cifar10checkpoint/poisongen/resnet18PPRM')
    torch.save(state, './Cifar10checkpoint/poisongen/resnet18PPRM/' +args.save +'_train_RN18_gp.pth')
...
def test(epoch, net):
    ...
    if not os.path.isdir('Cifar10checkpoint/poisongen/resnet18PPRM'):
        os.mkdir('Cifar10checkpoint/poisongen/resnet18PPRM')
    torch.save(state, './Cifar10checkpoint/poisongen/resnet18PPRM/' +args.save+'_test_RN18_gp.pth') 
...
for epoch in range(args.epochs):
    ...
    np.save('poisoned/resnet18PPRM/'+args.save+'_gpimage.npy', poisonimage_np)
    np.save('poisoned/resnet18PPRM/'+args.save+'_gplabel.npy', poisonlabel_np)
