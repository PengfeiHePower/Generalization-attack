...
# inner only consists of poison data
import numpy as np
import torch
...
for epoch in range(args.epochs):
    ...
    np.savetxt('sharp/resnet18CIP/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    ...
    plt.savefig('./figures/resnet18CIP/'+args.save+'_sharp.png')
    ...
    for batch_id, (images, targets) in enumerate(poisonloader):
        ...
        input_p = torch.clamp(input_p + plr_sch(epoch) * loss_grad, min=0.0, max=1.0)
        poisonimage_np[batch_id*128:(min((batch_id+1)*128,poisonsize))] = input_p.detach().cpu().numpy()


print('==> Data saving..')
np.save('poisoned/resnet18CIP/'+args.save+'_gpimage.npy', poisonimage_np)
np.save('poisoned/resnet18CIP/'+args.save+'_gplabel.npy', poisonlabel_np)