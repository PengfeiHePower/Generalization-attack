...
# restart the inner everytime, up to 200 epochs
...
parser.add_argument('--inner', default=200, type=int, help='iteration for inner')
...
def test(epoch, net):
    ...
    #         'epoch': epoch,
    # }
    # if not os.path.isdir('Cifar10checkpoint/poisongen/resnet18PPR-all'):
    #     os.mkdir('Cifar10checkpoint/poisongen/resnet18PPR-all')
    # torch.save(state, './Cifar10checkpoint/poisongen/resnet18PPR-all/' +args.save+'_test_RN18_gp.pth') 
...
for epoch in range(args.epochs):
    ...
    np.save('poisoned/resnet18PPR-all/'+args.save+'_gpimage.npy', poisonimage_np)
    np.save('poisoned/resnet18PPR-all/'+args.save+'_gplabel.npy', poisonlabel_np)
