# remove poison budget, start from a clean pre-trained model, test lr scheduler and maximize training loss directly
# inner only poison data
import numpy as np
import torch
...
parser.add_argument('--pr', default=1.0, type=float, help='poison rate')
...
parser.add_argument('--saveas', default='', type=str, help='save name')
#parser.add_argument('')
...
poisonsize = trainsize*args.pr
print('poisonstze:', poisonsize)
...
# sharpness.append(loss_all)
# np.savetxt('sharp/'+args.saveas +'_sharp.txt', np.array(sharpness))#store sharpness
...
# plt.savefig('./figures/'+args.saveas+'_sharp.png')
...
for epoch in range(args.epochs):
    ...
    # np.savetxt('sharp/resnet18CI/'+args.save+'_sharp.txt', np.array(sharpness))#store sharpness
    # plt.clf()#visualize sharpness
    # plt.figure(figsize=(8, 8))
    # plt.xlabel('epoch',fontsize=12,color=(0,0,0), weight='bold')
    # plt.ylabel('sharpness',fontsize=12,color=(0,0,0), weight='bold')
    ...
    # plt.yticks(size=12, weight='bold')
    # plt.plot(list(range(1,len(sharpness)+1)), sharpness)
    # plt.savefig('./figures/resnet18CI/'+args.save+'_sharp.png')
    ...
    for batch_id, (images, targets) in enumerate(poisonloader):
        ...
        poisonimage_np[batch_id*128:(batch_id*128+target_p.size(0))] = input_p.detach().cpu().numpy()
        poisonlabel_np[batch_id*128:(batch_id*128+target_p.size(0))] = target_p.detach().cpu().numpy()
        loss_all = loss_all + loss_s.item()* target_p.size(0)
        train_n = train_n + target_p.size(0)
    loss_all = loss_all/train_n
    sharpness.append(loss_all)
    np.savetxt('sharp/test_sharp.txt', np.array(sharpness))#store sharpness
    plt.clf()#visualize sharpness
    plt.figure(figsize=(8, 8))
    plt.xlabel('epoch',fontsize=12,color=(0,0,0), weight='bold')
    plt.ylabel('sharpness',fontsize=12,color=(0,0,0), weight='bold')
...
print('==> Data saving..')
np.save('poisoned/'+args.saveas+'_gpimage.npy', poisonimage_np)
np.save('poisoned/'+args.saveas+'_gplabel.npy', poisonlabel_np)